...
pub struct IngestTokenRequest {
    ...
    /// Sonde does not need historical session/version/OS state during token exchange.
...
///
/// The token exchange intentionally accepts only the current pseudonymous device identity. Current
/// session/version/OS values belong to telemetry records and their history is derived by Sonde.
...
#[cfg(test)]
mod tests {
    use super::{IngestScope, bind_event_device};
    ...
    fn signed_token_device_binding_blocks_identity_spoofing() {
        ...
        let mut forged = EventInput {
            ...
            session_id: Some("session-from-telemetry".into()),

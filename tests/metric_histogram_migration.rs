...
use sonde::database::{self, applications, query::insert};

#[tokio::test]
async fn migration_20_repairs_all_no_boundary_histogram_buckets() {
    let database = database::connect("sqlite::memory:").await.unwrap();

    // Apply through the histogram schema migration (migration 19), but deliberately stop before
    // the repair migration.
    database::Migrator::up(&database, Some(19)).await.unwrap();
    let (application_id, environment_id) =
        applications::create_application(&database, "Migration Test", "migration-test", None)
            .await
            .unwrap();

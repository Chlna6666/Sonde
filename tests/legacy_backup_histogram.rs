...
use sonde::{
    database::{self, app_repo, legacy_backup_repo, telemetry_repo},
...
async fn single_application_json_backup_preserves_histogram_population() {
    ...
    let (application_id, environment_id) =
        app_repo::create_application(&source, "Histogram App", "histogram-app", None)
            .await
            .unwrap();
    ...
    let backup = legacy_backup_repo::export_single_application(&source, &application_id)
        .await
        .unwrap()
        .unwrap();
    assert_eq!(backup.format_version, legacy_backup_repo::FORMAT_VERSION);
    ...
    let imported_application = legacy_backup_repo::import_single_application(&target, None, backup)
        .await
        .unwrap();
...
async fn full_system_json_backup_preserves_histogram_population_and_id() {
    ...
    let backup = legacy_backup_repo::export_full_system(&source)
        .await
        .unwrap();
    assert_eq!(backup.format_version, legacy_backup_repo::FORMAT_VERSION);
    ...
    let target = database::connect("sqlite::memory:").await.unwrap();
    database::migrate(&target).await.unwrap();
    legacy_backup_repo::restore_full_system(&target, backup)
        .await
        .unwrap();
...
#[test]
fn legacy_scalar_metric_without_histogram_is_still_accepted() {
    let metric: legacy_backup_repo::ExportedMetricPoint = serde_json::from_value(serde_json::json!({
        "environmentId": "prod",
        "name": "cpu.usage",
        "metricType": "gauge",
        "value": 0.5,
